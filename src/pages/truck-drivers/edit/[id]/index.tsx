...
import { getTruckDriverById, updateTruckDriverById } from 'apiSdk/truck-drivers';
...
function TruckDriverEditPage() {
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<TruckDriverInterface>(
    () => (id ? `/truck-drivers/${id}` : null),
    () => getTruckDriverById(id),
  );
  ...
  const handleSubmit = async (values: TruckDriverInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateTruckDriverById(id, values);
      mutate(updated);
      resetForm();
